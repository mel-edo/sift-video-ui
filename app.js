...
function renderHistory() {
  ...
  chips.innerHTML = h.map(q =>
    `<button class="chip" data-query="${q.replace(/"/g, '&quot;')}">${q}</button>`
  ).join('');

  // single delegated listener instead of inline onclick
  chips.querySelectorAll('.chip').forEach(btn => {
    btn.addEventListener('click', () => fillAndSearch(btn.dataset.query));
  });
...
function jumpTo(videoName, timestamp) {
  ...
  const column = document.getElementById('player-column');

  const newSrc = `/videos/${encodeURIComponent(videoName)}`;
 
  // if same video, just seek — no need to reload
  if (player.src === newSrc || player.src.endsWith(encodeURIComponent(videoName))) {
  ...
  list.innerHTML = results.map((r, i) => {
    ...
    return `
      <div class="result-card" style="animation-delay:${i * 0.04}s"
           onclick="jumpTo('${r.video_name}', ${r.timestamp})">
        ...
        <div class="card-bottom">
          ...
          <button class="jump-btn"
                  onclick="event.stopPropagation(); jumpTo('${r.video_name}', ${r.timestamp})">
